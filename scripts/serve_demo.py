...
def main() -> None:
    ...
    splits = {"train": list(corpus.iter_examples("train"))}
    loaders = RA._build_loaders(
        {"train": splits["train"][:8]}, specs,
        FeatureCache(args.cache_root / "whisper"), cfg)
    ...
    bins = json.loads((args.ckpt.parent / "bins.json").read_text()) \
        if (args.ckpt.parent / "bins.json").exists() else None
    if bins is None:
        raise SystemExit(
            f"missing {args.ckpt.parent / 'bins.json'} -- run export_bins.py "
            "so the live demo scores against the same thresholds as training")
