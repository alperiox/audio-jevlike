...
**Correction (I5b, final-review fix, 2026-09-20) — model selection.** Before this fix, dev metrics were computed and logged every epoch but TEST scoring used whatever model was left in memory once `run_arm`'s training loop ended — an arbitrary point on each arm's trajectory, for the Arm B vs Arm C comparison this whole grid exists to produce, at an expected effect size (~0.01 ECE) that arbitrariness could fully absorb. `run_arm` now selects an explicit **`best` checkpoint on dev NLL** (`train.loop.DEV_SELECTION_METRIC = "nll"`, reduced across question keys by `train.loop.dev_selection_score`, unweighted mean) each epoch, alongside an unconditionally-overwritten `last` checkpoint for resumability/inspection, and scores TEST from `best` — never from the loop's post-hoc in-memory state. **Why NLL and not ECE**, despite ECE being the phase's headline calibration metric: `expected_calibration_error` is a *binned* statistic (10 bins) and is genuinely noisy on a dev split this size; selecting the epoch whose dev ECE happens to be lowest and then reporting THAT epoch's test ECE is a subtle circularity — a low dev ECE can be partly bin-luck that does not carry over to the test split, biasing the reported test ECE optimistic relative to a selection metric that did not share its noise source with the reported one. Dev loss (the actual training objective) would sidestep that circularity even more cleanly, but is not what this phase measures, is not already available as a per-question quantity from `evaluate()`, and for Arm A (`brier_weight=0.0`) collapses to plain cross-entropy, which rewards confidence rather than calibration. NLL is the standard compromise for exactly this reason (Guo et al. 2017 fit/select temperature scaling on NLL, not ECE): a strictly proper scoring rule, sensitive to both calibration and discrimination, computed per-example rather than binned — far less noisy than ECE on a small split — and already present in `evaluate()`'s per-question output at zero extra cost. **Arm C is tied to Arm A's selected checkpoint by construction, not convention:** `run_derived_arm_c` locates Arm A's checkpoint via `_find_best_checkpoint`, which reads the fixed `best.pt` filename — the SAME file Arm A's own `run_arm` call scored its reported TEST metrics from. Arm C therefore cannot silently drift onto a different point of Arm A's trajectory than the one Arm A itself reports; if it did, the two arms would stop being "the same model plus a temperature," which is the exact property I9 established. See `tests/test_run_ablation.py::test_run_arm_scores_test_from_the_best_dev_epoch_not_the_last` (constructs a run where the best dev epoch is deliberately not the last, and asserts TEST scoring used the former) and the I5 fix report for fault-injection numbers.
...
**12 runs** (the 9-arm encoder grid + 3 text-only baseline arms, one per loss regime — §7.1 "Two baselines"). Fixed seeds, **Weights & Biases for every run**, checkpoints local (~1GB total — see the I5 correction in §10; each arm keeps only a dev-selected `best` and a rolling `last`, and the frozen sentence-transformer is excluded from what's saved), Drive as backup. Splits are **session-disjoint on IEMOCAP** (leave-one-session-out, enforced by `assert_speaker_disjoint`) but only **dialogue-disjoint and speaker-shared on MELD** — never utterance-random on either. See §4.1 for why MELD is not re-split and what that means for interpreting its results.

**Correction (I6, final-review fix, 2026-09-20):** "minutes each over cached features" was wrong by ~30x and is removed above rather than corrected in place, because the original number rested on an assumption (`ProsodiaModel.forward` running batched) that was false at the time it was written — `forward` un-batched `collate_batch`'s padded `(B, T, D)` batch into a per-example Python loop, measured at ~88% of forward's cost. The I6 fix batches `forward` (see `src/prosodia/model/prosodia.py` and its plan-doc Task 11 correction) and measured, on this machine (MPS, `in_dim=1024, d_model=256, B=16, T=150`): forward+backward per batch 122.9ms → 43.4ms (~2.8x). **This ~2.8x is an eval-mode upper bound, not a measured training-mode speedup** (F4, final-cleanup review, 2026-09-20): it was measured under `.eval()`, forced by an MPS SDPA-dropout limitation, and removes a fixed overhead that becomes a smaller fraction of a larger total once training-mode dropout and backward cost are included — the real training speedup is plausibly less than 2.8x, by an unbounded margin. Per-epoch/per-arm/grid wall time depends on dataset size (not yet run — feature extraction and the ablation grid are both still pending), so this section deliberately no longer states a wall-clock estimate; see the I6/I2 fix report for the full profiling detail instead of restating a number likely to go stale again.
...
| Checkpoints (12 arms × 2 files: `best.pt` + `last.pt`) | ~1 GB |
| **Total** | **~9.5 GB** of 73 GB free |

**Correction (I5, final-review fix, 2026-09-20):** the checkpoint line above was ~13 GB, derived from `run_arm` saving one full-`state_dict` file per epoch. Two independent fixes cut this by roughly 20x: (1) **I5a** — `train.loop.save_checkpoint` no longer saves `question_encoder._st`, the frozen `all-MiniLM-L6-v2` sentence transformer (`requires_grad_(False)` in `qencoder.py`, reconstructed from the HuggingFace hub by `QuestionEncoder.__init__` regardless of what any checkpoint restores). Measured at the grid's config (`in_dim=1024, d_model=256`): the full state_dict is 26.37M elements (86.1% — 22.71M elements — is the frozen encoder), so a checkpoint file (model + populated AdamW state) shrinks from ~135 MB to ~44 MB, a 67% reduction, with `load_checkpoint` now tolerating the expected absence of those keys (`strict=False`, but only for that specific prefix — any other missing/unexpected key still raises `RuntimeError`, so a real corruption is never silently swallowed). (2) **I5b** — `run_arm` no longer writes one checkpoint per epoch at all; it writes exactly two fixed files per arm, `best.pt` (rewritten whenever the dev-selection score improves) and `last.pt` (rewritten every epoch). 12 arms × 2 files × ~44 MB ≈ 1 GB total, comfortably inside budget. See `.superpowers/sdd/2026-09-19-prosodia-phase1/i5-fix-report.md` for the exact measurements.
...
- **Audio is capped at 30s for feature extraction (owner-reported fix, 2026-09-20).** WavLM's gated relative-position bias builds a T×T index tensor, so extraction memory scales with the *square* of clip length; a real extraction run over MELD died mid-corpus with `RuntimeError: Invalid buffer size: 13.85 GiB`. `FeatureExtractor.encode` (`src/prosodia/features.py`) now hard-truncates every clip to `DEFAULT_MAX_AUDIO_SECONDS` (30s) before it reaches any of the three encoder arms, uniformly — required by `assert_uniform_cache_coverage`, which needs every encoder's cache to cover an identical uid set. This affects 3 of 13,706 MELD utterances (0.02%). Two of the three are MELD segmentation artifacts, not genuinely long turns — the clip boundary is simply wrong, not the duration: `dia38_utt4.wav` (304.9s of audio for the transcript *"Oh it's great, it's a role on"*) and `dia220_utt0.wav` (235.1s for *"What's that smell?"*). The third, in the 20–41s range, is a genuinely long turn. `scripts/extract_features.py` prints every truncated uid at the end of a run rather than truncating silently. At the default 30s, this cap coincides exactly with `WhisperFeatureExtractor`'s own fixed 30s mel window (see the R1 note in `features.py`), so it is a no-op for the whisper arm and changes behavior only for wavlm/prosody, which have no such internal bound.
